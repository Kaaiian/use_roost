...
def init_model(orig_atom_fea_len, args):
...
#     ensemble(model_name, args.fold_id, train_set, test_set,
#              args.ensemble, orig_atom_fea_len, args)


def ensemble(model_name, fold_id, dataset, test_set,
             ensemble_folds, fea_len, args):
    """
    ...
    if not args.evaluate:
        ...
        for run_id in range(ensemble_folds):
            ...
            writer = SummaryWriter(log_dir=(f"runs/{model_name}_"
                                            "{date:%d-%m-%Y_%H-%M-%S}").format(
                                                date=datetime.datetime.now()))

            experiment(model_name, fold_id, run_id, args,
...
def test_ensemble(model_name, fold_id, ensemble_folds, hold_out_set, fea_len, args):
    ...
    if ensemble_folds == 1:
        df.to_csv(index=False,
                  path_or_buf=(f"results/test_results_{model_name}.csv"))
    else:
        df.to_csv(index=False,
