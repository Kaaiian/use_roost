...
class CompositionData(Dataset):
    ...
    def __init__(self, data_path, fea_path):
        ...
        if self.df.shape[-1] == 2:
            ...
            self.df.insert(0, 'crystal_id', range(self.df.shape[0]))

        self.df.columns = ['crystal_id', 'composition', 'target']
        not_pure = [True] * self.df.shape[0]

        for i, composition in enumerate(self.df['composition']):
            elements, weights = parse(composition)
            if len(elements) == 1:
                not_pure[i] = False

        if sum(not_pure) < self.df.shape[0]:
            n_original = self.df.shape[0]
            n_not_pure = sum(not_pure)
            n_pure = n_original - sum(not_pure)
            print(f'Removed {n_pure} pure elements.\n{n_not_pure}'
                  f'/{n_original} formulae remain.')
        self.df = self.df[not_pure]
